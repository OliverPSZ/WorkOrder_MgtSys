...
ws=wb.sheets[0]
ws2=wb.sheets[2]
x=ws2.range('J1').options(numbers=int).value
...
def invoice_print(invoices,ot):
    if 'IMT' in ot[0]:
        for step in range(len(invoices)):
            if (ws.range('P'+str(step+5)).value=='-') & (invoices[step]!='-') & (('*' not in ot[step]) and ('#' not in ot[step])):
                tmp2=ot[step].split('-')
                tmp3=tmp2[0]+'-'+tmp2[1]
        ...
        for step in range(len(invoices)):
            if (ws.range('N'+str(step+5)).value=='-') & (invoices[step]!='-') & (('*' not in ot[step]) and ('#' not in ot[step])):
                tmp2=ot[step].split('-')
                tmp3=tmp2[0]+'-'+tmp2[1]
                for j in range(len(invoices)):
                    if (ws2.range('A'+str(j+3)).value==tmp3):
                ...
                if 'A' not in fact[y]:
                    if (sw1[y]!='*') and (sw1[y]!='#'):
                        ...
                        if (sw2[y]!='SIN COTIZACIÓN') & (sw2[y]!='-') & (sw2[y]!='SIN COTIZACION') & (sw2[y]!='SIN COT / CANCELADA'):
                            ...
                            if sw1f in w or sw2f in w or "C0"+sw2f[2:len(sw2f)] in w:
                                invoices[y]='A'+str(x)
                            else:
                    ...
                    if invoices[y] is None or invoices[y]=='-':
                            invoices[y]='-'
        x=x+1
        ws2.range('J1').value=x-1
       
    invoice_print(invoices,sw1)
